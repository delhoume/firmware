...
FASTEPD bbep;
#endif
#include "../lib/bb_epaper/Fonts/ItcMachine_80.h"
...
=======
int png_to_epd(const uint8_t *pPNG, int iDataSize, const uint8_t *pPNG_old,
               int iDataSize_old) {
  int iPlane, rc = -1;
  PNG *png = new PNG();
...
>>>>>>> Stashed changes
        ...
        if (pPNG_old) { // decode the old image to do a partial update
          if (png->openRAM((uint8_t *)pPNG_old, iDataSize_old, png_draw) ==
              PNG_SUCCESS) {
            Log_info("%s [%d]: preparing plane 1 for partial update\r\n",
                     __FILE__, __LINE__);
            bbep.startWrite(PLANE_1); // 'old' data is written to plane 1
            if (png->getBpp() ==
                2) { // tell draw code to handle old 2bpp image differently
...
=======
void display_show_image(uint8_t *image_buffer, int data_size,
                        uint8_t *image_buffer_old, int data_size_old)
// void display_show_image(uint8_t *image_buffer, bool reverse, int data_size)
>>>>>>> Stashed changes
{

  display_show_msg(nullptr, WIFI_INTERNAL_ERROR); // TEST);
  delay(10000);
  bool isPNG = true;
...
=======
  if (isPNG == true && data_size < DEFAULT_IMAGE_SIZE) {
    Log_info("Drawing PNG");
    iRefreshMode =
        png_to_epd(image_buffer, data_size, image_buffer_old, data_size_old);
  } else // uncompressed BMP or Group5 compressed image
  {
    if (*(uint16_t *)image_buffer == BB_BITMAP_MARKER) {
      // G5 compressed image
...
>>>>>>> Stashed changes
    ...
    bbep.writePlane(PLANE_0); // send image data to the EPD
    //        iUpdateCount = 0; // reset the update count when woken by button
    //        press of BMP file is shown
  }
...
=======
  ...
  //    if (iUpdateCount == 1) {
  //        Log_info("%s [%d]: Forcing fast refresh (not partial) since the logo
  //        was just shown\r\n", __FILE__, __LINE__); iRefreshMode =
  //        REFRESH_FAST;
  //    }
  Log_info("%s [%d]: EPD refresh mode: %d\r\n", __FILE__, __LINE__,
...
void display_show_msg(uint8_t *image_buffer, MSG message_type) {
  ...
  case TEST: {
   ...
   for (int idx = 0; idx < 15; ++idx) {
      ...
      bbep.getStringBox(buffer, &rect);
      int color = BBEP_BLACK;
      int numoffset = (barwidth - rect.w) / 2;
      // top
      bbep.fillRect(idx * (barwidth + baroffset), 0, barwidth, idx, color);
      bbep.drawString(buffer, idx * (barwidth+ baroffset) + numoffset, textoffset);

      // bottom
      bbep.fillRect(idx * (barwidth+ baroffset), bbep.height() - idx, barwidth, idx, color);
      bbep.drawString(buffer, idx * (barwidth + baroffset) +  numoffset, bbep.height() - textoffset);

      // left
      bbep.fillRect(0, idx * (barwidth+ baroffset), idx, barwidth, color);
      bbep.drawString(buffer, textoffset, idx * (barwidth + baroffset) + numoffset);

      // right
      bbep.fillRect(bbep.width() - idx, idx * (barwidth+ baroffset), idx, barwidth, color);
      bbep.drawString(buffer, bbep.width() - textoffset, idx * (barwidth + baroffset) + numoffset);
    }
...
=======
    ...
    //      bbep.println("A B C D E F G H I Y A B C D E F G H I Y A B C D E");
    //      bbep.println("a b c d e f g h i y a b c d e f g h i y a b c d e");
    bbep.setCursor(300, 240);
    bbep.setFont(itcmachine80);
    bbep.println("awAW");
  } break;
  default:
    break;
...
>>>>>>> Stashed changes
  ...
  case MAC_NOT_REGISTERED: {
    ...
    Paint_DrawMultilineText(0, y_start, message.c_str(), width, font_width,
                            BBEP_BLACK, BBEP_WHITE, Roboto_20, true);
  } break;
  default:
    break;
  }
