...
void png_draw(PNGDRAW *pDraw) {
  ...
  uint8_t uc, ucMask, src, *s, *d,
      *pTemp = bbep.getCache(); // get some scratch memory (not from the stack)
  ...
  if (!pDraw->pUser) {
    ...
    if (*(int *)pDraw->pUser >
        1) {                // draw 2bpp data as 1-bit to use for partial update
      ...
      ucMask = (*(int *)pDraw->pUser == 0) ? 0x40
                                           : 0x80; // lower or upper source bit
      for (x = 0; x < pDraw->iWidth; x++) {
        uc <<= 1;
        if (src & ucMask) {
          uc |= 1; // high bit of source pair
...
const uint8_t ucTwoBitFlags[256] = {
    };
...
void png_draw_count(PNGDRAW *pDraw) {
  ...
  if (pDraw->y > 430)
    return; // Workaround to ignore the icon in the lower left corner
 ...
 *        only 2 unique colors. This will allow us to use partial
 * (non-flickering) updates on these images.
...
int png_to_epd(const uint8_t *pPNG, int iDataSize) {
  ...
  if (rc == PNG_SUCCESS) {
    if (png->getWidth() != bbep.width() || png->getHeight() != bbep.height()) {
      ...
      if (png->getBpp() == 1 || png_count_colors(png, pPNG, iDataSize) ==
                                    2) { // 1-bit image (single plane)
...
void trmnlDrawMultilineTextCenteredAt(int lines, const char *text[], int cx, int cy, int justify) {
  bbep.drawLine(cx, 0, cx, bbep.height() - 1, BBEP_BLACK);
  bbep.drawLine(0, cy, bbep.width() - 1, cy, BBEP_BLACK); 
 int maxwidth = -1;
  BB_RECT bbox;
  // first pass to compute max if needed
  if (justify) {
    for (int l = 0; l < lines; ++l) {
      // compute position
      bbep.getStringBox(text[l], &bbox);
      if (bbox.w > maxwidth)
        maxwidth = bbox.w;
    }
  }
  //  y xpacing must be the same
  int linespacing = 20; // TODO: replace by real value
  int height = lines * linespacing;
  int y = cy - (height / 2) + linespacing;
  int x = cx - maxwidth / 2; // default justify = 1;
  for (int l = 0; l < lines; ++l) {
    // compute position
    bbep.getStringBox(text[l], &bbox);
    if (justify == 0)
      ...
      x = cx + maxwidth / 2 - bbox.w;
    bbep.drawString(text[l], x, y);
    y += linespacing;
  }
}
/**
...
void display_show_image(uint8_t *image_buffer, int data_size, bool bWait)
{
  ...
  } else // uncompressed BMP or Group5 compressed image
  {
    ...
    } else { // This work-around is due to a lack of RAM; the correct method
             // would be to use loadBMP()
  ...
  if (refresh_seconds >= 30 * 60 && iRefreshMode == REFRESH_PARTIAL) {
    // For users who set updates 30 minutes or longer, use the "fast" update to
    // prevent ghosting
    Log_info("%s [%d]: Forcing fast refresh (not partial) since the TRMNL "
             "refresh_rate is set to > 30 min\n",
             __FILE__, __LINE__);
...
void display_show_msg(uint8_t *image_buffer, MSG message_type) {
  ...
  case WIFI_FAILED: {
    ...
    bbep.loadG5Image(wifi_failed_qr, bbep.width() - 66 - 40, 40, BBEP_WHITE,
                     BBEP_BLACK);
  } break;
  ...
  case MSG_TOO_BIG: {
    ...
    bbep.println(string2);
    String string3 =
        String(MAX_IMAGE_SIZE) + String(" bytes each and 1 or 2-bpp");
  ...
  case TEST: {
    const char string1[] = "The image file from this URL is too large.";
    strcpy(filename,
           "a very long complex filename with many parts and components");
    if (strlen(filename) > 40) {
      filename[40] = 0; // truncate and add elipses
      strcat(filename, "...");
    }
    ...
    const char *text[4] = {string1, filename, string2, string3.c_str()};

    trmnlDrawMultilineTextCenteredAt(4, text, bbep.width() / 2, bbep.height() / 4, 1);
    trmnlDrawMultilineTextCenteredAt(4, text, bbep.width() / 2, bbep.height() / 2, 0);
   trmnlDrawMultilineTextCenteredAt(4, text, bbep.width() / 2,bbep.height() * 3 / 4, 2);
  } break;
  default:
    break;
  }
...
void display_show_msg(uint8_t *image_buffer, MSG message_type,
  ...
  if (message_type == WIFI_CONNECT) {
    ...
    if (!apiDisplayResult.response.maximum_compatibility) {
      bbep.refresh(REFRESH_FAST,
                   true); // newer panel can handle the fast refresh
    } else {
  ...
  case WIFI_CONNECT: {
    ...
    bbep.println(string1);
    const char string2[] =
        "Connect your phone or computer to TRMNL WiFi network";
    bbep.getStringBox(string2, &rect);
    bbep.setCursor((bbep.width() - rect.w) / 2, 386);
    bbep.println(string2);
    ...
    bbep.setCursor((bbep.width() - rect.w) / 2, -1);
    bbep.print(string3);
    bbep.loadG5Image(wifi_connect_qr, bbep.width() - 40 - 66, 40, BBEP_WHITE,
                     BBEP_BLACK); // 66x66 QR code
  } break;
  case MAC_NOT_REGISTERED: {
    UWORD y_start = 340;
    UWORD font_width = 18; // DEBUG
    Paint_DrawMultilineText(0, y_start, message.c_str(), width, font_width,
                            BBEP_BLACK, BBEP_WHITE, nicoclean_8 /*Roboto_20*/,
                            true);
  } break;
  default:
    break;
